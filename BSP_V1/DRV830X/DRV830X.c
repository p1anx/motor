...
// 1. 为DRV8301对象静态分配内存（通常定义为全局变量）
// 使用 __attribute__((aligned(4))) 确保内存对齐，这对Cortex-M内核等架构很重要
...
// #ifdef  IS_ODRIVE
    ...
    handle->spiHandle = &DRV8301_SPI;

    handle->spiHandle->Init.DataSize = SPI_DATASIZE_16BIT;
    handle->spiHandle->Init.CLKPolarity = SPI_POLARITY_LOW;
    handle->spiHandle->Init.CLKPhase = SPI_PHASE_2EDGE;
    handle->spiHandle->Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_8;
// #endif
...
void DRV8301_enable(DRV8301_Handle handle)  // loop222.5
{
    // Enable driver
...
// end of file
void DRV8301_SetGain(DRV8301_Handle handle, int gain) {
    switch (gain) {
    case 10:
        DRV8301_setShuntAmpGain(handle, DRV8301_ShuntAmpGain_10VpV);
        break;
    case 20:
        DRV8301_setShuntAmpGain(handle, DRV8301_ShuntAmpGain_20VpV);
        break;
    case 40:
        DRV8301_setShuntAmpGain(handle, DRV8301_ShuntAmpGain_40VpV);
        break;
    case 80:
        DRV8301_setShuntAmpGain(handle, DRV8301_ShuntAmpGain_80VpV);
        break;
    default:
        DRV8301_setShuntAmpGain(handle, DRV8301_ShuntAmpGain_10VpV);
        break;
...
int DRV8301_GetGain(DRV_SPI_8301_Vars_t drv8301_vars) {
    switch (drv8301_vars.Ctrl_Reg_2.GAIN) {
...
DRV_Status_t DRV8301_Setup(DRV8301_CONFIG_t *pDRV8301_Config) {
    ...
    DRV8301_SetGain(drv8301, pDRV8301_Config->gain);
    // DRV8301_setShuntAmpGain(drv8301, DRV8301_ShuntAmpGain_80VpV);
    DRV8301_setupSpi(drv8301, &drv8301_vars);
    HAL_Delay(100);
    const int gain = DRV8301_GetGain(drv8301_vars);
    printf("[INFO] DRV8301_Gain = %d\n", gain);
    return DRV_OK;

}

void drv8301_example1(void) {
    DRV8301_CONFIG_t drv8301_Config;
    drv8301_Config.gain = 40; // 10 20 40 80
    DRV8301_Setup(&drv8301_Config);

}

void drv8301_example0(void) {
