...
void MG513_Init(Motor_InitStruct *motor)
{
    ...
    // TB6612_ENABLE();
    // tb6612_pwm_setup(TB6612_PWM1_PERIOD,
    // speed); tb6612_direction(direction);
...
#ifdef DEBUG_PRINT
    // printf("overflow = %d\n",
    // motor->pulse_count); my_printf("overflow =
    // %d\n", g_overflow_encoder_count);
#endif
    return 0;
}
...
float read_motor_pulses(Motor_InitStruct *motor)
{
    ...
    {
        // count =
        // (float)(0xffff-__HAL_TIM_GET_COUNTER(motor->encoder_tim))
        // +
        // g_overflow_encoder_count*(ENCODER_COUNT_PERIOD);
        count =
            (float)(0xffff - __HAL_TIM_GET_COUNTER(motor->encoder_tim)) + (motor->pulse_count) * (ENCODER_COUNT_PERIOD);
        // count =
        // (float)(__HAL_TIM_GET_COUNTER(motor->encoder_tim))
        // + (motor->pulse_count) *
        // (ENCODER_COUNT_PERIOD); count =
        // (float)(-__HAL_TIM_GET_COUNTER(motor->encoder_tim))
        // +
    ...
    // my_printf("motor pulse count = %d, motor
    // overflow = %d, count = %.2f\n",
    // __HAL_TIM_GetCounter(&encoder_htim),
    // motor->pulse_count, count);
    return count;
}
#endif

// float read_motor_pulses(Motor_InitStruct*
// motor){
//   float count;
//   //(float) is important for solution of
//   negative value
...
//   overflow = %d, count = %.2f\n",
//   __HAL_TIM_GetCounter(&encoder_htim),
//   motor->pulse_count, count); return count;
// }
...
float read_motor_speed(Motor_InitStruct *motor)
{
    ...
    motor_speed = (float)((delta_count) / (4 * MOTOR_REDUCTION_RATIO * ENCODER_RESOLUTION * PID_UPDATE_TIME));
    motor_lastCount = motor_totalCount;
    // my_printf("total count = %.2f, delta = %.2f
    // overflow_cnt = %d\n", motor_totalCount,
    // delta_count, motor->pulse_count);
    return motor_speed;
}
#endif
...
// float anglePID_Update(PID_Controller *pid,
...
float Incremental_PID_Update(PID_Controller *pid, float setpoint, float measurement)
{
    float error = setpoint - measurement;
...
float positional_pid_position_loop(Motor_InitStruct *motor)
{
...
#ifdef DEBUG_PRINT
    printf("\n");
    my_printf("actual_position = %.2f, "
              "target_position = %.2f\n",
              actual_position, pid_pos_init->target_position);
    ...
    // read_motor_speed(motor); float
    // motor_postion = read_motor_position(motor);
    // // printf("%.2f\n", motor_postion);
    printf("%.2f\n", actual_position);
#endif
    // static int tim_count = 0;
    ...
    //     printf("%.2f\n", actual_position);
    //     printf("overflow = %d\n",
    //     motor->pulse_count); printf("direction
    //     = %d\n",
...
float incremental_pid_position_loop(Motor_InitStruct *motor)
{
    ...
    Incremental_PID_Update(pid_pos_init, pid_pos_init->target_position, actual_position);
...
#ifdef DEBUG_PRINT
    my_printf("actual_speed = %.2f, target_speed "
              "= %.2f\n",
              actual_speed, pid_vel_init->target_velocity);
    my_printf("direction = %d\n", direction);
...
void incremental_pid_velocity_loop(Motor_InitStruct *motor)
{
    ...
    Incremental_PID_Update(pid_vel_init, pid_vel_init->target_velocity, actual_speed);
    // my_printf("out = %.2f\n",
    // pid_vel_init.output);
    ...
    {
        tim_count = 0;
        printf("position = %d, overflow = %d\n", read_motor_position(motor), motor->pulse_count);
        printf("\n");
    }
    tim_count++;
...
void dual_pid_position(Motor_InitStruct *motor)
{
    ...
    // 位置环计算期望速度
    float target_vel = PID_Update(&dual_pid_pos_init, target_position, current_pos);
...
#ifdef DEBUG_PRINT
    my_printf("targe_pos = %.2f, current_pos = "
              "%.2f, output = %.2f\n",
              target_position, current_pos, output);
    my_printf("target_speed = %.2f,current_speed "
              "= %.2f\n",
              target_vel, current_vel);
    my_printf("innerIntegral = %.2f\n", dual_pid_vel_init.integral);
    // my_printf("innerError = %.2f\n",
    // InnerError0); my_printf("outerTarget =
    // %.2f, outerActual = %.2f\n", OuterTarget,
    // OuterActual); my_printf("outerError =
    // %.2f\n", OuterError0);
    // my_printf("outerIntegral = %.2f\n",
    // OuterErrorInt); my_printf("outerOut =
    // %.2f\n", OuterOut); my_printf("deltaCount =
    // %.2f\n", delta_count);
    my_printf("\n");
#endif
#ifdef DEBUG_PLOT_PID
...
// #define   BIT_00 POSITION_INCREMENT_00 #define
// BIT_01                POSITION_INCREMENT_01
// #define   BIT_11 POSITION_INCREMENT_11 #define
// BIT_10                POSITION_INCREMENT_10
void timer_control_position_qam16(Motor_InitStruct *motor)
{
    static int tim_count = 0;
    static float bit[16] = {BIT_0000, BIT_0001, BIT_0010, BIT_0011, BIT_0110, BIT_1010, BIT_1110, BIT_0100,
                            BIT_1000, BIT_1100, BIT_0101, BIT_1001, BIT_1101, BIT_0111, BIT_1011, BIT_1111};
...
void timer_control_position_qam16_sequence(Motor_InitStruct *motor)
{
    ...
    {
        tim_count = 0;
        motor->pid_pos_init->target_position = bit_base + qam16_base * (bit_count++) + circle_base * circle_count;
        // bit_count++;
        if (bit_count == 16)
        {
            bit_count = 0;
...
#define POSITION_INCREMENT_11 0000.0f + (5000.0f * 4 + BIAS) / 2 + (5000.0f * 4 + BIAS) / 4
...
int add_bias[4] = {OFFSET_COMPASTION, OFFSET_COMPASTION, OFFSET_COMPASTION, OFFSET_COMPASTION};
void timer_control_position_qam_sequence(Motor_InitStruct *motor)
{
    ...
    if (tim_count > bit_update_time / PID_UPDATE_TIME)
    {
        ...
        {
            motor->pid_pos_init->target_position = BIT_00_1 + add_bias[0] * bias_count + circle_count * circle_base;
        }
        if (bit[bit_count] == BIT_01_1)
        {
            motor->pid_pos_init->target_position = BIT_01_1 + add_bias[1] * bias_count + circle_count * circle_base;
        }
        if (bit[bit_count] == BIT_11_1)
        {
            motor->pid_pos_init->target_position = BIT_11_1 + add_bias[2] * bias_count + circle_count * circle_base;
        }
        if (bit[bit_count] == BIT_10_1)
        {
            motor->pid_pos_init->target_position = BIT_10_1 + add_bias[3] * bias_count + circle_count * circle_base;
        }
        bit_count++;
        bias_count++;
        // if (bias_count == 38) {
...
void timer_control_position_qam_sequence_without_offset(Motor_InitStruct *motor)
{
    static int tim_count = 0;
...
void timer_control_position_qam_adaptive(Motor_InitStruct *motor)
{
    ...
    // float bit_delay_time[4] = {1-0.835,
    // 1-1.115, 1-0.925, 1-1.125}; float
    // bit_delay_time[4] = {1.5, 1, 0.5, 0.2};
    float bit_delay_time[4] = {1 - 0.96, 1 - 1.02, 1 - 1.02, 1 - 1.01};

    if (tim_count >= (bit_update_time - bit_delay_time[delay_count]) / PID_UPDATE_TIME)
    {
        // if (delay_count == 4) {
        //   delay_count = 0;
        // } else {
        //     delay_count++;
        //     }
        printf("delay time = %.2f, time_count = "
               "%d\n",
               bit_delay_time[delay_count], tim_count);
...
void tb6612_setPWM(Motor_InitStruct *motor, float duty)
{
    ...
    // __HAL_TIM_SetAutoreload(motor->pwm_tim,
    // 4095);
    __HAL_TIM_SET_PRESCALER(motor->pwm_tim, PWM_TIM_PSC - 1);
    __HAL_TIM_SetAutoreload(motor->pwm_tim, PWM_RESOLUTION - 1);
    __HAL_TIM_SetCompare(motor->pwm_tim, motor->pwm_channel, duty - 1);
    // __HAL_TIM_SetAutoreload(motor->pwm_tim,
    // TB6612_PWM1_PERIOD - 1);
    // __HAL_TIM_SetCompare(motor->pwm_tim,
    // motor->pwm_channel, duty - 1);
}
...
void key_control_position(Motor_InitStruct *motor)
{
    ...
    //   POSITION_INCREMENT; printf("target
    //   position =
    //   %.2f\n",motor->pid_pos_init->target_position);
