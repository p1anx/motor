...
float read_motor_pulses(Motor_InitStruct* motor){
  ...
  if(__HAL_TIM_IS_TIM_COUNTING_DOWN(motor->encoder_tim)){
  // count = (float)(0xffff-__HAL_TIM_GET_COUNTER(motor->encoder_tim)) + g_overflow_encoder_count*(ENCODER_COUNT_PERIOD);
  count = (float)(0xffff-__HAL_TIM_GET_COUNTER(motor->encoder_tim)) + (motor->pulse_count) * (ENCODER_COUNT_PERIOD);
  // count = (float)(-__HAL_TIM_GET_COUNTER(motor->encoder_tim)) + motor->pulse_count*(ENCODER_COUNT_PERIOD);
  }
  else{
  // count =  -(float)__HAL_TIM_GET_COUNTER(motor->encoder_tim) + (g_overflow_encoder_count+1)*(ENCODER_COUNT_PERIOD);//(g_overflow_encoder_count+1)补偿启动时，由于--的起始值为-1,加1做补偿
  count =  -(float)__HAL_TIM_GET_COUNTER(motor->encoder_tim) + (motor->pulse_count+1)*(ENCODER_COUNT_PERIOD);//(g_overflow_encoder_count+1)补偿启动时，由于--的起始值为-1,加1做补偿
...
void positional_pid_velocity_loop(Motor_InitStruct* motor){
      ...
      float actual_speed = read_motor_speed(motor);
      PID_Controller* pid_vel_init = motor->pid_vel_init;
...
PID_Controller dual_pid_pos_init = {
    .Kp = 0.4f,
    .Ki = 0.00f,
    .Kd = 2.00f,
    .output_max = 5000.0f,
...
PID_Controller dual_pid_vel_init = {
    .Kp = 3.0,
    .Ki = 0.10f,
    .Kd = 0.00f,
    .output_max = 5000.0f,
...
void dual_pid_position(Motor_InitStruct* motor){
  ...
  float target_position = motor->pid_pos_init->target_position;
    ...
    // my_printf("outerTarget = %.2f, outerActual = %.2f\n", OuterTarget, OuterActual);
    // my_printf("outerError = %.2f\n", OuterError0);
...
void timer_control_position_qam16(Motor_InitStruct* motor) {
  ...
  static float bit[16] = {BIT_0000,BIT_0001, BIT_0010, BIT_0011, BIT_0110, BIT_1010, BIT_1110,BIT_0100,BIT_1000,BIT_1100,BIT_0101,BIT_1001,BIT_1101,BIT_0111,BIT_1011,BIT_1111};
