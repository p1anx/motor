...
float Encoder_getVelocityRPM(Encoder_t *encoder)
{
    switch (encoder->EncoderType)
    {
        float velocity;
    case EncoderType_AS5600:
        velocity = AS5600_getVelocityRPM(&encoder->AS5600);
        encoder->angle = encoder->AS5600.angle;
        return velocity;
    case EncoderType_MT6835:
        velocity = MT6835_GetVelocityRPM(&encoder->mt6835);
        // velocity = MT6835_GetVelocityDegrees(&encoder->mt6835);
        encoder->angle = encoder->mt6835.angle;
        return velocity;
    default:
        printf("please choose a valid encoder type for velocity rpm!\n");
        break;
    }
    return 0;
