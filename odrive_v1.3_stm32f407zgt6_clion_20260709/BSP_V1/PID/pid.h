#ifndef PID_H_
#define PID_H_

#ifdef __cplusplus
extern "C"
{
#endif

#include "foc.h"
// #include "time_utils.h"
#define pid_tim htim1
#define PID_UPDATE_T 10 // MS

    /**
     * PID controller structure
     */
    typedef struct
    {
        float P;           //!< Proportional gain
        float I;           //!< Integral gain
        float D;           //!< Derivative gain
        float output_ramp; //!< Maximum speed of change of the output value
        float limit;       //!< Maximum output value
        float update_t;       //!< Maximum output value

        float integral;          //!< last integral component value
        float error;             //!< last tracking error value
        unsigned int timestamp; //!< Last execution timestamp
        float output;            //!< last pid output value
    } PIDController_t;

    /**
     * Initialize PID controller
     * @param pid - Pointer to PID controller structure
     * @param P - Proportional gain
     * @param I - Integral gain
     * @param D - Derivative gain
     * @param ramp - Maximum speed of change of the output value
     * @param limit - Maximum output value
     */
    void PIDController_init(PIDController_t *pid, float P, float I, float D, float ramp, float limit);
    void PID_init(PIDController_t *pid);

    /**
     * Calculate PID controller output
     * @param pid - Pointer to PID controller structure
     * @param error - Current error value
     * @return PID controller output
     */
    float PIDController_update(PIDController_t *pid, float error);

#ifdef __cplusplus
}
#endif

#endif /* PID_H_ */
