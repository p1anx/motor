...
CurrentSense_t CurrentSense_epc9193 = {
  .gain = 50,
  .r_sample = 0.001f,
...
Motor_t Motor_3508 = {
  ...
  .freq_velocityLoop = CONFIG_PWM_FREQUENCY / 2,
  .freq_positionLoop = CONFIG_PWM_FREQUENCY / 4,
...
PID_t PID_3508 = {
  ...
  .PID_iVelocity = {
    .P = 0.01f, // 0.005 current velocity
    .I = 0.5f, // 0.005
    .D = 0,
    .output_ramp = 0,
    .limit = 10, //A
...
//   // 1. 将pCurrentSense指向的结构体内容复制到pMainCurrentSense指向的结构体
//   if (pCurrentSense != NULL && pCurrentSenseVar != NULL) {
//     *pCurrentSense = *pCurrentSenseVar;  // 关键：解引用后直接赋值
//   }
//   motor->pCurrentSense = pCurrentSense;
//   if (pDRV803xConfig) {
//     motor->pCurrentSense->gain = pDRV803xConfig->gain;
//   }
...
void ConfigMotor_Motor(Motor_t* motor, const Motor_t* motor_u, ConfigSTM32_t* pConfigSTM32) {
  ...
  motor->isODrive = motor_u->isODrive;
}
...
void ConfigMotor_DRV830X(Motor_t* motor, CurrentSense_t* pCurrentSense) {
  motor->drv830xConfig.gain = pCurrentSense->gain;
  // pCurrentSense->gain = pDRV830xConfig->gain;

}
void ConfigMotor_Check(Motor_t* motor) {
...
void ConfigMotor_PID(Motor_t* motor, const PID_t* PID, ConfigSTM32_t* pConfigSTM32) {
  ...
  motor->PID_VelocityAngle = PID->PID_VelocityAngle;
...
//   CurrentSense_t* pCurrentSenseVar = &CurrentSense_epc9193;
