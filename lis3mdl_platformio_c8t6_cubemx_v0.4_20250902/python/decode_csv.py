...
def B_filter_read_csv():
    ...
    # dataBy = pd.to_numeric(df, errors="coerce").dropna().values

    data = np.array(dataBy)
    # data = dataBy
    fs = 320
    print(f"len = {len(data)}, type = {type(data)}")

    N = len(data)
    B = np.fft.fft(data)[:N//2]
    B = np.abs(B)
    B_freq = np.fft.fftfreq(N, 1/fs)[:N//2]
    t = np.linspace(0, len(data), len(data))
    plt.figure(figsize=(8, 6))
    plt.subplot(211)
    plt.plot(t, data, label = "Bx")
    plt.subplot(212)
    plt.plot(B_freq, B)
