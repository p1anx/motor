...
void ESP8266_ClientInit(UART_HandleTypeDef *huart)
{
    ...
    char cmd_rst[] = "AT+RST\r\n";
    char cmd_linkWifi[] = "AT+CWJAP=\"BlackWind\",\"19950402\"\r\n";
    ...
    ESP8266_RUN_MODE();
    ESP8266_Reset();
    printf("[INFO] Quiting in TransparentMode\n");
    ESP8266_SendCmd_NotResponse(esp_uart, cmd_exit, timeout);
    printf("[INFO] Setting WorkMode\n");
    ESP8266_SendCmd_NotResponse(esp_uart, cmd_workMode, timeout);
    HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_2);
    // ---
    // printf("[INFO] Resetting\n");
    // ESP8266_SendCmd_NotResponse(esp_uart, cmd_rst, long_timeout);
    // ---
    // ESP8266_SendCmd_NotResponse(esp_uart, cmd_linkWifi, timeout);
    ESP8266_SendCmd_NotResponse(esp_uart, cmd_mux, timeout);
    printf("[INFO] Connecting Server\n");
    esp_status = ESP8266_SendCmd(esp_uart, cmd_linkSever, "CONNECT", long_timeout);
    if (esp_status != ESP8266_OK)
    {
        printf("=================================================\n");
        printf("[ERROR] SERVER isn't connected, SYSTEM IS RESET!\n");
        printf("[NOTE] Check Server is started!\n");
        printf("=================================================\n");
        system_reset();
    }
    // ESP8266_SendCmd_NotResponse(esp_uart, cmd_linkSever, long_timeout);
    printf("[INFO] Setting TransparentMode On\n");
    ESP8266_SendCmd_NotResponse(esp_uart, cmd_transparentModeOn, timeout);
    printf("[INFO] Starting to Send Message\n");
