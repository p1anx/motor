...
class Serial:
    ...
    def readline(self):
        try:
            ...
            self.ser.close()  # 关闭串口
    def readline_sync(self, sync_flag = 'start'):
        start_flag = ''
        data_count = 0
        try:
            while True:
                if self.ser.in_waiting > 0:  # 检查是否有可读数据
                    # data = ser.readline().decode('utf-8').strip()  # 读取一行并解码
                    while start_flag != sync_flag:
                        print("waiting start flag....", end='\r', flush=True)
                        data = self.ser.readline().decode('utf-8', errors='ignore').strip()  # 忽略错误字符
                        #1.
                        # data = data.split(",")
                        # start_flag = data[0]
                    ...
                    data = self.ser.readline().decode('utf-8', errors='ignore').strip()  # 忽略错误字符
                    if data == 'end':
                        print(f"recieve is over, data num = {data_count} \n")
                        return self.Bx_lists, self.By_lists, self.Bz_lists, self.pos_lists
                    if not data:
                        continue
                    ...
                    self.By_lists.append(float(By))
                    self.Bz_lists.append(float(Bz))
                    # self.pos_lists.append(float(pos))
    ...
    #                     if not data:  # 如果接收到空数据，跳过
    #                         continue
    #                     data_parts = data.split(",")
    #                     if len(data_parts) > 0:  # 确保有数据
    #                         start_flag = data_parts[0].strip()  # 去除空白字符
    #                     else:
    #                         continue  # 如果分割后没有数据，继续等待
                    
    #                 # 读取实际数据
    #                 data = self.ser.readline().decode('utf-8', errors='ignore').strip()
    #                 if not data:  # 如果数据为空，继续循环
    #                     continue
                        
    #                 # 解析数据，添加错误处理
    #                 try:
    #                     data_parts = [x.strip() for x in data.split(",") if x.strip()]  # 过滤空字符串
    #                     if len(data_parts) < 4:  # 确保有足够的数据
    #                         print(f"数据长度不足，期望4个值，实际{len(data_parts)}个: {data_parts}")
    #                         continue
    #                     dataB = [float(x) for x in data_parts]
...
def test_getDeltaB():
    ...
    uart_rx = Serial(port='/dev/ttyUSB1', baudrate=115200, data_n=1000)
    # uart.start()
...
def test_2():
    ...
    try:
        ...
        if rx_strings:
            print(f"前 5 条: {rx_strings[:5]}")
            print(f"最后 5 条: {rx_strings[-5:]}")
def test_getDeltaB_with_position():
    # uart = UARTTransceiver('/dev/ttyACM0', 115200)
    uart_rx = Serial(port='/dev/ttyACM0', baudrate=115200, data_n=50000)
    start_flag = 'start'
    # uart.start()
    ...
    try:
        ...
        # count = 0
        # while True:
            data1,data2,data3 = uart_rx.readline_sync('start')
            m.save_3data_to_csv(data1, data2, data3, "data.csv")
            m.plot(data1)
            
            plt.show()
            # uart_rx.readline_sync('start')
            #1.
            # data1_deltaB = max(data1) - min(data1)
            # data2_deltaB = max(data2) - min(data2)
            # data3_deltaB = max(data3) - min(data3)
...
def test_read3data():
    # uart = UARTTransceiver('/dev/ttyACM0', 115200)
    uart_rx = Serial(port='/dev/ttyACM1', baudrate=115200, data_n=1000)
    # uart.start()
...
def test_thread_rx(port):
    # serial = UARTTransceiver(port=port, baudrate=115200)
    # serial.start()
    uart = UARTTransceiver(port, 115200)
    uart.start()
    buffer = []
