...
def save_4data_to_csv(data1, data2, data3, pos, filename='data.csv'):
    """
    将四个数据保存到CSV文件中
    """
    # 创建数据字典
    data = {
        'timestamp': [datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')],  # 添加时间戳
        'data1': [data1],
        'data2': [data2],
        'data3': [data3],
        'pos': [pos]
...
#     # 使用pandas的to_csv方法，mode='a'表示追加模式
#     df.to_csv(filename, 
#             #   mode='a',                    # 追加模式
#               header=not file_exists,      # 如果文件不存在则写入表头，否则不写表头
#               index=False)                 # 不保存行索引
    
#     print(f"数据已追加保存到 {filename}")
...
def read_from_csv(fileName):
    ...
    return data.values
def save_4data_to_csv(pos, data1, data2, data3, fileName = 'data.csv'):
    data = [pos, data1, data2, data3]
    save_to_csv(data, fileName)

def save_3data_to_csv(pos, data1, data2, fileName = 'data.csv'):
    data = [pos, data1, data2]
    save_to_csv(data, fileName)
# # 使用示例
# save_4data_to_csv(1.1, 2.2, 3.3, 4, 'data.csv')
# save_4data_to_csv(5.5, 6.6, 7.7, 8, 'data.csv')
# save_4data_to_csv(9.9, 10.10, 11.11, 12, 'data.csv')