...
def test_getDeltaB_with_position():
    # uart = UARTTransceiver('/dev/ttyACM0', 115200)
    uart_rx = Serial(port='/dev/ttyACM0', baudrate=115200, data_n=500)
    start_flag = 'start'
    # uart.start()
    ...
    try:
        ...
        # count = 0
        # while True:
            data1,data2,data3,pos = uart_rx.readline_sync('start')
            m.save_4data_to_csv(pos, data1, data2, data3, "data.csv")
            plt.plot(pos, data1)
            plt.show()
