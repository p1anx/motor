...
void currentSense_3currentCalibration(CurrentSense_t *currentSense)
{
    ...
    if (currentSense->isCalibrating)
    {
        ...
        if (sample_i == sample_num)
        {
            ...
            printf("current offset: ia = %f, ib = %f, ic = %f\n", currentSense->offset_a, currentSense->offset_b, currentSense->offset_c);
            return ;
        }
    }
    return ;
...
void currentSense_ADC_Callback(ADC_HandleTypeDef *hadc)
{
    ...
    if (hadc->Instance == ADC1)
    {
        ...
        Clark_Park_Transform(currentSense.i_a, currentSense.i_b, &currentSense.i_d, &currentSense.i_q, motor.e_angle);
        ...
        //3.
        // motor.CurrentSense.i_a = LowPassFilter(&motor.lpf_ia, currentSense.i_a);
        // motor.CurrentSense.i_b = LowPassFilter(&motor.lpf_ib, currentSense.i_b);
        //4.
        // motor.CurrentSense.i_a = lpf_process(&motor.lpf_ia0, currentSense.i_a);
        // motor.CurrentSense.i_b = lpf_process(&motor.lpf_ib0, currentSense.i_b);
...
void test_adc_callback(ADC_HandleTypeDef *hadc)
{
    ...
    {
        HAL_GPIO_TogglePin(GPIOD, GPIO_PIN_12);
        fTest_adc_buffer[1] = (float)(HAL_ADCEx_InjectedGetValue(&hadc1, ADC_INJECTED_RANK_1))*3.3*2/65535;
        adc_ok = 1;

    }
}

void HAL_ADCEx_InjectedConvCpltCallback_with_multirate_controller(ADC_HandleTypeDef *hadc)
{
    if (hadc->Instance == ADC1)
    {
        // HAL_GPIO_TogglePin(GPIOD, GPIO_PIN_12);
