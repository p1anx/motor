...
void Motor_GetCurrentIdIq(Motor_t *motor) {
  ...
  float* ia = &motor->pCurrentSense->i_a;
  float* ib = &motor->pCurrentSense->i_b;
  // float* ic = &motor->pCurrentSense->i_c;
...
void Motor_velocityClosedLoop(Motor_t *motor, float ref_velocity) {
  ...
  monitor.ref_velocity = motor->ref;
