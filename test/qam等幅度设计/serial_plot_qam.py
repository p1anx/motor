...
def serial_plot_only():
    ...
    try:
        ...
        Bz_lists = []
        sample_per_bit = 310
        rxData_num = sample_per_bit * 10 * 2 * 10 / 10

        # saved_file_path = "./data_after_offset_t/qam4_1Hz_1s_7/"
        saved_file_path = "./fft_sample_test/"
            ...
            if ser.in_waiting > 0:  # 检查是否有可读数据
                ...
                if len(data) > 28:
                    ...
                    if Bx_lists_n > rxData_num:
                        pd.DataFrame(Bx_lists).to_csv(
                            saved_file_path + "Bx.csv", index=False, header=False
                        )
                        pd.DataFrame(By_lists).to_csv(
                            saved_file_path + "By.csv", index=False, header=False
                        )
                        pd.DataFrame(Bz_lists).to_csv(
                            saved_file_path + "Bz.csv", index=False, header=False
                        )
