/*
 * FOCMotor.c
 * Ported from: https://github.com/simplefoc/Arduino-FOC/blob/master/src/common/base_classes/FOCMotor.cpp
...
float FOCMotor_shaftAngle(FOCMotor_t *motor) {
  ...
  if(!motor->sensor) return motor->shaft_angle;
...
float FOCMotor_shaftVelocity(FOCMotor_t *motor) {
  ...
  if(!motor->sensor) return 0;
  return LowPassFilter_calculate(&motor->LPF_velocity, Sensor_getVelocity(motor->sensor));
}

// Virtual function placeholders - these should be implemented by derived structs
...
int FOCMotor_initFOC(FOCMotor_t *motor, float zero_electric_offset, Direction_t sensor_direction) {
  // This is a virtual function in C++ - should be implemented by BLDCMotor
  (void)motor;
