...
void currentSense_3current(CurrentSense_t *currentSense)
{
    ...
    currentSense->i_a = HAL_ADCEx_InjectedGetValue(currentSense->adc, ADC_INJECTED_RANK_3)*scales - currentSense->offset_a;
    currentSense->i_b = HAL_ADCEx_InjectedGetValue(currentSense->adc, ADC_INJECTED_RANK_2)*scales - currentSense->offset_b;
    currentSense->i_c = HAL_ADCEx_InjectedGetValue(currentSense->adc, ADC_INJECTED_RANK_1)*scales - currentSense->offset_c;
...
void test_adc_callback(ADC_HandleTypeDef *hadc)
{
    ...
    {
        HAL_GPIO_TogglePin(GPIOD, GPIO_PIN_12);
        fTest_adc_buffer[1] = (float)(HAL_ADCEx_InjectedGetValue(&hadc1, ADC_INJECTED_RANK_1))*3.3/4096;
        adc_ok = 1;
