...
def line_plot():
    ...
    eight_points = np.linspace(start, end, 7)
    for i, p in enumerate(eight_points):
        print(f"point {i}: {p:.6f}")
    # 数轴示意：线段 + 8 个等分点（编号 0..7）
    plt.figure(figsize=(10, 2.5))
    plt.plot([start, end], [0, 0], "k-", linewidth=1.5, label="[start, end]")
    plt.scatter(
        eight_points,
        np.zeros_like(eight_points),
        c="tab:red",
        s=55,
        zorder=3,
        label="8 points",
    ...
    margin = 0.02 * (end - start)
    plt.xlim(start - margin, end + margin)
    plt.ylim(-0.4, 0.4)
    plt.yticks([])
    plt.xlabel("position")
    plt.title("Eight equally spaced points (start → end)")
