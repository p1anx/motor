...
def plot_qam_constellation(
    ...
    # 设置坐标轴范围：I/Q 同范围，保证 1 单位在 x、y 方向等长
    i_vals = np.real(constellation)
    q_vals = np.imag(constellation)
    max_axis = max(np.max(np.abs(i_vals)), np.max(np.abs(q_vals)), 1e-9) * 1.2
    ...
    plt.legend()
    plt.tight_layout()
    ax.set_aspect("equal", adjustable="box")  # tight_layout 后再次固定等比例
    # plt.show()
