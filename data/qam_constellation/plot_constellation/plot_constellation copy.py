...
def plot_qam_constellation(file,segments_index = None, fs=284, start=231, step=270, g_n_point=10, column=2, saved_foler = "data_img"):
    ...
    if segments_index is None:
        ...
        return False
    n_segments = len(segments_index) - 1
    ...
    for i in range(n_segments):
        # plt.subplot(211)
        # data_segment = data[start : start + step]
        data_segment = data[segments_index[i]: segments_index[i + 1]]
        x = range(segments_index[i], segments_index[i + 1])
        # # 段太短时跳过，避免 filtfilt 报错
        # if len(data_segment) <= 12:
        #     start = start + step
        #     continue
...
def test_plot_qam_constellation():
    ...
    #1.ok
    # csv_file = f"data/1p25/sin_2s2s2s2s2s1p25s2p75s2s_40s.csv"
    # segments_index = [0, 500, 1048, 1557, 2106, 2614, 3155, 3497, 4202, 4721, 5266, 5794, 6305, 6816, 7361, 7701, 8430]

    #2.ok
    csv_file = f"data/1p25/8qam/data_20260326_8qam.csv"
    # segments_index = [0, 531, 1040, 1399, 2115, 2439, 3154, 3688, 4203, 4747,5256, 5609,6314,6659, 7365, 7910,8424,8961,9474,9811]
    segments_index = [0, 531, 1040, 1399, 2115, 2480, 3154, 3688, 4203, 4747,5256, 5609,6314,6671, 7365, 7910,8424,8939,9474,9811]
    ...
    plot_qam_constellation(
        csv_file, segments_index=segments_index, fs=255, start=start, step=step, g_n_point=g_n_point, saved_foler=img_folder
    )

    # 绘制并保存星座图（在 plot_qam_constellation_filtered 末尾已生成 figure）
def test_plot_16qam_constellation():

    img_folder = "./data_20260409"
    time_str = get_current_time()
    g_n_point = 8
    ...
    #2.ok
    # file_name = "data_16qam_80s.csv"
    file_name = "data_20260409_044116.csv"

    csv_file = f"{img_folder}/{file_name}"
    # segments_index = [0, 531, 1040, 1399, 2115, 2439, 3154, 3688, 4203, 4747,5256, 5609,6314,6659, 7365, 7910,8424,8961,9474,9811]
    # segments_index = [0, 651, 1119-50, 1778-50, 2115-50, 2866-50,3219-50, 3556-50, 4209, 4660-50, 5303-50, 5733-50, 6309, 6698-50, 7434-50, 8154-50, 8476-50, 9135-50, 9549-50, 10193-50]
    # segments_index = [1778-50, 2115-50, 2866-50,3219-50, 3556-50, 4209, 4660-50, 5303-50, 5733-50, 6309, 6698-50, 7434-50, 8154-50, 8476-50, 9135-50, 9549-50, 10193-50]
    # segments_index = [0, 5000]

    segments_index = [581, 1086, 1173, 1620, 1746, 2153, 2357, 2658]
    delta_list = []
    for i in range(len(segments_index)//2):
        delta = segments_index[i*2+1] - segments_index[i*2]
        print(f'delta = {delta}')
        delta_list.append(delta)
    min_delta = min(delta_list)
    print(f'min_delta = {min_delta}')
    for i in range(len(segments_index)//2):
        segments_index[i*2+1] = segments_index[i*2] + min_delta
    print(f'segments_index = {segments_index}')
    
    # print(f'len = {len(segments_index)}')
