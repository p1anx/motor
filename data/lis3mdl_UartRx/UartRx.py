...
def send_cmd_to_single_ports(
    ...
    port1: 串口号，例如 'COM3' 或 '/dev/ttyUSB0'
...
def collect_and_plot_three(
    ...
    t0 = time.time()
    now_t = datetime.now()
    t_time = now_t.strftime("%Y%m%d_%H%M%S")
    csv_file = f"{csv_path}/data_{t_time}.csv"
    try:
    ...
    plt.savefig(f"{csv_path}/data_{t_time}")
    plt.show()
...
def plot_fft(
    ...
    label : 图例标签
    title : 图表标题
    save_path : 若提供路径，则保存图片到该路径
    show : 是否调用 plt.show() 显示图像

    Returns
    -------
    freq : 频率数组 (Hz)，只包含正频率部分
    magnitude : 对应的幅度谱
    """
    ...
    # 取正频率部分（单边谱）
    half_n = n // 2
    freq = np.fft.fftfreq(n, d=1.0 / fs)[:half_n]
    magnitude = np.abs(fft_vals[:half_n]) / n * 2  # 归一化并补偿单边能量

    # 绘图
    plt.figure()
    plt.plot(freq, magnitude, label=label, linewidth=0.8)
...
def plot_fft_three_channels(
    ch1: list[float] | np.ndarray,
    ch2: list[float] | np.ndarray,
    ch3: list[float] | np.ndarray,
    fs: float,
    save_dir: str | None = None,
    show: bool = True,
) -> None:
    """
    对三通道数据分别绘制 FFT 幅度谱，子图排列。

    Parameters
    ----------
    ch1, ch2, ch3 : 三个通道的数据
    fs : 采样频率 (Hz)
    save_dir : 若提供目录路径，保存图片到该目录下
    show : 是否显示图像
    """
    channels = [
        (ch1, "ch1"),
    ...
    fig.suptitle("Three-Channel FFT Magnitude Spectrum", fontsize=14)
    ...
    for ax, (data, ch_name) in zip(axes, channels):
        ...
        half_n = n // 2
        freq = np.fft.fftfreq(n, d=1.0 / fs)[:half_n]
        magnitude = np.abs(fft_vals[:half_n]) / n * 2

        ax.plot(freq, magnitude, label=ch_name, linewidth=0.6)
        ax.set_xlabel("Frequency (Hz)")
        ax.set_ylabel("Magnitude")
        ax.set_title(f"{ch_name} FFT")
        ax.legend()
    ...
    if save_dir is not None:
        ...
        plt.savefig(f"{save_dir}/fft_three_ch_{now}.png")
    if show:
        plt.show()


def collect_and_plot_fft(
    port: str,
    fs: float,
    baudrate: int = 115200,
    encoding: str = "utf-8",
    ...
    encoding : 字符编码
    duration : 采集时长（秒）
    csv_dir : 若提供目录路径，CSV 文件保存到该目录下
    save_dir : 若提供目录路径，图片保存到该目录下（时域和 FFT 各一张）
    show : 是否显示图像

    Returns
    -------
    ch1, ch2, ch3 : 三个通道的采集数据
    """
    ser = serial.Serial(port=port, baudrate=baudrate, timeout=0.1)
    ...
    n = len(ch1)
    print(f"采集到 {n} 个样本，采样频率 fs={fs} Hz，时长 {n / fs:.3f} s")

    # ---------- 保存 CSV ----------
    if csv_dir is not None:
        try:
            csv_path = f"{csv_dir}/data_{t_time}.csv"
            df = pd.DataFrame({"ch1": ch1, "ch2": ch2, "ch3": ch3})
    ...
    fig1, axes1 = plt.subplots(3, 1, figsize=(10, 9), sharex=True)
    fig1.suptitle("Time Domain", fontsize=14)

    for ax, (data, ch_name) in zip(axes1, [(ch1, "ch1"), (ch2, "ch2"), (ch3, "ch3")]):
    ...
    plt.tight_layout()
    if save_dir is not None:
        fig1.savefig(f"{save_dir}/time_{t_time}.png")
    if show:
        plt.show()

    # ---------- FFT 图 ----------
    fig2, axes2 = plt.subplots(3, 1, figsize=(10, 9), sharex=False)
    fig2.suptitle("FFT Magnitude Spectrum", fontsize=14)

    for ax, (data, ch_name) in zip(axes2, [(ch1, "ch1"), (ch2, "ch2"), (ch3, "ch3")]):
        ...
        ax.plot(freq, magnitude, linewidth=0.6)
        ax.set_xlabel("Frequency (Hz)")
        ax.set_ylabel("Magnitude")
        ax.set_title(f"{ch_name} FFT")
        ax.legend([ch_name])
    ...
    plt.tight_layout()
    if save_dir is not None:
        fig2.savefig(f"{save_dir}/fft_{t_time}.png")
    if show:
        plt.show()
...
if __name__ == "__main__":
    ...
    #     baudrate1=115200,
    #     baudrate2=2000000,
    #     duration=10,             # 接收并绘图前采集 5 秒
    # )
    print('wating')
    csv_folder = "data_20260723"
    uartSinglePortPlot(
        tx_port1="/dev/ttyUSB0",  # 第一个发送端口
        rx_port="/dev/ttyUSB0",   # 实际回传数据的端口（根据你的硬件修改）
        cmd="start",
        ...
        duration=10*30*5,             # 接收并绘图前采集 5 秒
    )
    # collect_and_plot_fft(port="/dev/ttyUSB0",fs=500, baudrate= 5e5, csv_dir=csv_folder, save_dir=csv_folder)