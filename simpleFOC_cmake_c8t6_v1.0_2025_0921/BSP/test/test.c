...
void test_alignSensor(void) {
  // BLDCDriver3PWM_enable(); // This function doesn't exist anymore - needs driver instance
  alignSensor();
  float target_angle[LEN] = {0, 90, 180};
  float kp = 0.133;
  float sensor_angle;
...
void test_pid(void) {
  ...
  _delay(1000);
  while (1) {
    float angle = FOCMotor_shaftAngle(&motor);

    float velocity = FOCMotor_shaftVelocity(&motor);
    printf("angle = %f\n", angle);
    printf("vel = %f\n", velocity);
    // _delay(1000);
