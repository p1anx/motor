    ...
    while (1)
    {
        ...
        if (motor.isEnable)
        {
            ...
            if (currentSense.adc_flag == 1)
            {
                ...
                // i_q = LowPassFilter(&lpf_a, i_q);
                // UART_SendFloat(6,i_d, i_q, currentSense.i_a, currentSense.i_b, i_beta, i_alpha);
                // UART_SendFloat(8,motor.target, motor.velocity, i_d, i_q, motor.currentSense->i_a, motor.currentSense->i_b, i_beta, i_alpha);

                // UART_SendFloat(2,motor.currentSense->i_a, motor.currentSense->i_b);
                // UART_SendFloat(2,i_d, i_q);
                // UART_SendFloat(8,motor.target, motor.velocity, i_d, i_q, motor.currentSense->i_a, motor.currentSense->i_b, i_beta, i_alpha);
                // UART_SendFloat(3,i_d, i_q, delta_t);
                UART_SendFloat(1, delta_t);
                // UART_SendFloat(1,motor.currentSense->i_a);
                // UART_SendFloat(1,i_q);
                last_t = now_t;
                */
                ...
                last_t = now_t;
                // UART_SendFloat(1,delta_t);
                UART_SendFloat(1,motor.currentSense->i_q);
                currentSense.adc_flag = 0;
