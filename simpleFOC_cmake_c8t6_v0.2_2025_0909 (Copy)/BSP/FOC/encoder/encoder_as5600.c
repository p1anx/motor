...
bool AS5600_InitWithPin(AS5600_t *sensor, I2C_HandleTypeDef *hi2c,
                        uint8_t address, GPIO_TypeDef *port, uint8_t pin) {
...
bool AS5600_IsConnected(AS5600_t *sensor) {
  ...
  HAL_StatusTypeDef status =
      HAL_I2C_IsDeviceReady(sensor->hi2c, sensor->address << 1, 2, 100);
...
void AS5600_SetDirection(AS5600_t *sensor, uint8_t direction) {
  ...
  if (sensor->directionPin != AS5600_SW_DIRECTION_PIN &&
      sensor->directionPort != NULL) {
    HAL_GPIO_WritePin(sensor->directionPort, (1 << sensor->directionPin),
...
uint8_t AS5600_GetDirection(AS5600_t *sensor) {
  ...
  if (sensor->directionPin != AS5600_SW_DIRECTION_PIN &&
      sensor->directionPort != NULL) {
    sensor->direction =
        HAL_GPIO_ReadPin(sensor->directionPort, (1 << sensor->directionPin));
  }
...
uint16_t AS5600_RawAngle(AS5600_t *sensor) {
  ...
  if ((sensor->directionPin == AS5600_SW_DIRECTION_PIN) &&
      (sensor->direction == AS5600_COUNTERCLOCK_WISE)) {
...
bool AS5600_IncreaseOffset(AS5600_t *sensor, float degrees) {
  if (!sensor)
    return false;
  return AS5600_SetOffset(sensor,
                          (sensor->offset * AS5600_RAW_TO_DEGREES) + degrees);
}
...
int32_t AS5600_GetCumulativePosition(AS5600_t *sensor, bool update) {
  ...
  // 整圈顺时针旋转？
  // 少于半圈
  if ((sensor->lastPosition > 2048) &&
      (value < (sensor->lastPosition - 2048))) {
...
uint8_t AS5600_ReadReg(AS5600_t *sensor, uint8_t reg) {
  ...
  HAL_StatusTypeDef status =
      HAL_I2C_Mem_Read(sensor->hi2c, sensor->address << 1, reg,
...
uint8_t AS5600_WriteReg(AS5600_t *sensor, uint8_t reg, uint8_t value) {
  ...
  HAL_StatusTypeDef status =
      HAL_I2C_Mem_Write(sensor->hi2c, sensor->address << 1, reg,
...
void AS5600_Example_ReadSpeed(void) {
  // 获取角速度（度/秒）
  float speedDegrees =
      AS5600_GetAngularSpeed(&magneticEncoder, AS5600_MODE_DEGREES, false);

  // 获取角速度（弧度/秒）
  float speedRadians =
      AS5600_GetAngularSpeed(&magneticEncoder, AS5600_MODE_RADIANS, true);

  // 获取转速（RPM）
  float speedRPM = AS5600_GetAngularSpeed(&magneticEncoder, AS5600_MODE_RPM, 1);

  printf("Speed: %.2f deg/s, %.4f rad/s, %.2f RPM\n", speedDegrees,
         speedRadians, speedRPM);
}
