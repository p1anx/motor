...
void test_adc(void) {
  ...
  while (1) {
    ...
    if (HAL_IS_BIT_SET(HAL_ADC_GetState(&currentSense_adc1),
...
void CurrentSense_Init(CurrentSense_t *currentSense) {
  currentSense->gain = AMPLIFIER_GAIN;
...
void CurrentSense_2ADC_Init(CurrentSense_t *currentSense) {
  ...
  currentSense->adc_resolution = 4096; // 12bit

  float sum_a = 0, sum_b = 0, sum_c = 0;
  int samples = 100;
  for (int i = 0; i < samples; i++) {
    sum_a += CurrentSense_readRawCurrent(currentSense->adc);
    sum_b += CurrentSense_readRawCurrent(currentSense->adc);
    delay_ms(1);
  }
  currentSense->offset_a = sum_a / samples;
  currentSense->offset_b = sum_b / samples;
  printf("current offset: a=%.2f v, b=%.2f v\n",
         currentSense->offset_a / currentSense->adc_resolution * 3.3,
         currentSense->offset_b / currentSense->adc_resolution * 3.3);
...
void CurrentSense_InitADC(CurrentSense_t *currentSense) {
  ...
  currentSense->R_sample = R_SAMPLE;
  currentSense->adc = &currentSense_adc1;
  currentSense->adc_resolution = ADC_RESOLUTION; // 16bit
  currentSense->isCalibrating = 1;
  currentSense->adc_flag = 0;
  currentSense->isReady = false;
  __HAL_ADC_ENABLE_IT(currentSense->adc, ADC_IT_JEOC);
  if (HAL_ADCEx_InjectedStart_IT(currentSense->adc) != HAL_OK) {
...
void CurrentSense_InitADC_Regular(CurrentSense_t *currentSense) {
  ...
  currentSense->R_sample = R_SAMPLE;
  currentSense->adc = &currentSense_adc1;
  currentSense->adc_resolution = 65535; // 16bit
  currentSense->isCalibrating = 1;
  currentSense->adc_flag = 0;
  currentSense->isReady = false;
  currentSense->isSetIc = 1;
  currentSense_pollingCalibrate(currentSense);
}
...
uint16_t CurrentSense_Sampling(CurrentSense_t *currentSense) {
  ...
  if (HAL_IS_BIT_SET(HAL_ADC_GetState(currentSense->adc),
                     HAL_ADC_STATE_REG_EOC)) {
...
int CurrentSense_Scan(CurrentSense_t *pCurrentSense, const int channel_num,
                      int *rawValue) {
...
int currentSense_get2PhaseCurrent(CurrentSense_t *pCurrentSense) {
  ...
  float scale = 3.3f / ((float)pCurrentSense->adc_resolution *
                        pCurrentSense->R_sample * pCurrentSense->gain);
...
int currentSense_get2PhaseVoltage(CurrentSense_t *pCurrentSense) {
  ...
  CurrentSense_Scan(pCurrentSense, 2, adc_value);
  const float scale =
      3.3f / ((float)pCurrentSense->adc_resolution * pCurrentSense->gain);
...
int currentSense_3PhaseCurrentCalibrate(CurrentSense_t *pCurrentSense) {
  ...
  const float scale = 3.3f / ((float)pCurrentSense->adc_resolution *
                              pCurrentSense->gain * pCurrentSense->R_sample);
  const int sample_n = 1000;
...
int currentSense_get3RawPhaseVoltage(CurrentSense_t *pCurrentSense,
                                     float *rawVoltage) {
  int adc_value[3];
  CurrentSense_Scan(pCurrentSense, 3, adc_value);
  const float scale = 3.3f / ((float)pCurrentSense->adc_resolution);
...
float CurrentSense_getRawCurrent_v0(CurrentSense_t *currentSense) {
  ...
  current =
      adc_value * 3.3 / (4096 * currentSense->R_sample * currentSense->gain);
  return current;
}
...
void CurrentSense_calibrate(CurrentSense_t *pCurrentSense) {
  ...
  printf("current offset: a=%.2f v, b=%.2f v, c=%.2f v\n",
         pCurrentSense->offset_a / pCurrentSense->adc_resolution * 3.3,
         pCurrentSense->offset_b / pCurrentSense->adc_resolution * 3.3,
         pCurrentSense->offset_c / pCurrentSense->adc_resolution * 3.3);
}
...
float CurrentSense_readCurrent(CurrentSense_t *pCurrentSense) {
  ...
  float adc_value = adc_rawValue * 3.3 /
                    (pCurrentSense->adc_resolution * pCurrentSense->R_sample *
                     pCurrentSense->gain);
  return adc_value;
}
float CurrentSense_read3Current(CurrentSense_t *pCurrentSense) {
  float scales = 3.3 / (pCurrentSense->adc_resolution *
                        pCurrentSense->R_sample * pCurrentSense->gain);
  pCurrentSense->i_a = (CurrentSense_readRawCurrent(pCurrentSense->adc1) -
                        pCurrentSense->offset_a) *
                       scales;
  pCurrentSense->i_b = (CurrentSense_readRawCurrent(pCurrentSense->adc2) -
                        pCurrentSense->offset_a) *
                       scales;
  pCurrentSense->i_c = (CurrentSense_readRawCurrent(pCurrentSense->adc3) -
                        pCurrentSense->offset_a) *
                       scales;
  return 0;
}
...
float CurrentSense_getRawCurrent(CurrentSense_t *currentSense) {
  ...
  current = adc_value * 3.3 /
            (currentSense->adc_resolution * currentSense->R_sample *
             currentSense->gain);
  return current;
}
...
// float CurrentSense_getCurrentDQ(BLDCMotor_t *motor, CurrentSense_t
// *currentSense)
// {
...
//     // 7.1 test angle
//     // float angle = _normalizeAngle(_electricalAngle(motor.angle,
...
void currentSense_3currentCalibration(CurrentSense_t *currentSense) {

  {
    ...
    // const int sample_phase = -1;
    const int sampleA_phase = 1;
    const int sampleB_phase = 1;
    if (sample_i < sample_num) {
      ia_sum += (sampleA_phase)*HAL_ADCEx_InjectedGetValue(currentSense->adc,
                                                           ADC_INJECTED_RANK_1);
      ib_sum += (sampleB_phase)*HAL_ADCEx_InjectedGetValue(currentSense->adc,
                                                           ADC_INJECTED_RANK_2);
      ic_sum +=
          HAL_ADCEx_InjectedGetValue(currentSense->adc, ADC_INJECTED_RANK_3);
    ...
    sample_i++;
    if (sample_i == sample_num) {
      currentSense->offset_a =
          (float)ia_sum / sample_num * 3.3 / (currentSense->adc_resolution);
      currentSense->offset_b =
          (float)ib_sum / sample_num * 3.3 / (currentSense->adc_resolution);
      currentSense->offset_c =
          (float)ic_sum / sample_num * 3.3 / (currentSense->adc_resolution);
      // currentSense->offset_a = (float)ia_sum / sample_num
      // *3.3/(currentSense->R_sample*currentSense->gain*currentSense->adc_resolution);
      // currentSense->offset_b = (float)ib_sum / sample_num
      // *3.3/(currentSense->R_sample*currentSense->gain*currentSense->adc_resolution);
      // currentSense->offset_c = (float)ic_sum / sample_num
      // *3.3/(currentSense->R_sample*currentSense->gain*currentSense->adc_resolution);
      printf("current offset: ia = %f, ib = %f, ic = %f\n",
             currentSense->offset_a, currentSense->offset_b,
             currentSense->offset_c);
      return;
    }
  }
...
void currentSense_currentCalibrationVoltage(CurrentSense_t *currentSense) {
  CurrentSense_ADC_Type adcType = currentSense->adc_type;
  if (adcType == ADCType_2ADC) {
    currentSense_2currentCalibrationVoltage(currentSense);
  }
  else if (adcType == ADCType_3ADC) {
...
void currentSense_2currentCalibrationVoltage(CurrentSense_t *currentSense) {

  {
    ...
    if (sample_i == sample_num) {
      ...
      currentSense->offset_b =
          (float)ib_sum / sample_num * 3.3 / (currentSense->adc_resolution);
      printf("Voltage_offset: va = %fv, vb = %fv\n",
             currentSense->offset_a, currentSense->offset_b);
...
void currentSense_3currentCalibrationVoltage(CurrentSense_t *currentSense) {

  {
    ...
    if (sample_i == sample_num) {
      ...
      // currentSense->offset_c = (float)ic_sum / sample_num
      // *3.3/(currentSense->R_sample*currentSense->gain*currentSense->adc_resolution);
      printf("Voltage_offset: va = %fv, vb = %fv, vc = %fv\n",
             currentSense->offset_a, currentSense->offset_b,
             currentSense->offset_c);
      currentSense->isCalibrating = 0;
    }
...
void currentSense_3current(CurrentSense_t *currentSense) {
  const float scales = -3.3f / ((float)currentSense->adc_resolution *
                                currentSense->R_sample * currentSense->gain);
  const int sampleA_phase = 1;
  const int sampleB_phase = 1;

  currentSense->i_a = (sampleA_phase)*HAL_ADCEx_InjectedGetValue(
                          currentSense->adc, ADC_INJECTED_RANK_1) *
                          scales -
                      currentSense->offset_a;
  currentSense->i_b = (sampleB_phase)*HAL_ADCEx_InjectedGetValue(
                          currentSense->adc, ADC_INJECTED_RANK_2) *
                          scales -
                      currentSense->offset_b;
  currentSense->i_c =
      HAL_ADCEx_InjectedGetValue(currentSense->adc, ADC_INJECTED_RANK_3) *
...
void currentSense_currentVoltage(CurrentSense_t *currentSense) {
  CurrentSense_ADC_Type adcType = currentSense->adc_type;
  if (adcType == ADCType_2ADC) {
    currentSense_2currentVoltage(currentSense);
  }
  else if (adcType == ADCType_3ADC) {
...
void currentSense_3currentVoltage(CurrentSense_t *currentSense) {
  const float scales = 3.3f / ((float)currentSense->adc_resolution *
                                currentSense->R_sample * currentSense->gain);
  const float v_scales = 3.3f / ((float)currentSense->adc_resolution);
  const float i_scales = 1/(currentSense->R_sample*currentSense->gain);
  const int sampleA_phase = 1;
  const int sampleB_phase = 1;
  // float v_bias =
  // currentSense->offset_a = 2.03f;
  // currentSense->offset_b = 2.03f;

  currentSense->v_a = HAL_ADCEx_InjectedGetValue(currentSense->adc, ADC_INJECTED_RANK_1) * v_scales;
  currentSense->v_b = HAL_ADCEx_InjectedGetValue(currentSense->adc, ADC_INJECTED_RANK_2) * v_scales;
  currentSense->v_c = HAL_ADCEx_InjectedGetValue(currentSense->adc, ADC_INJECTED_RANK_3) * v_scales;
  currentSense->i_a = (currentSense->v_a - currentSense->offset_a)*i_scales;
  currentSense->i_b = (currentSense->v_b - currentSense->offset_b)*i_scales;
  currentSense->i_c = (currentSense->v_c - currentSense->offset_c)*i_scales;
}
...
void currentSense_getCurrentDQ_NotFilter(CurrentSense_t *currentSense) {
  ...
  Clarke_Transform(currentSense->i_a, currentSense->i_b, &i_alpha, &i_beta);
  Park_Transform(i_alpha, i_beta, _normalizeAngle(motor.e_angle), &i_d, &i_q);
  currentSense->i_d = i_d;
  currentSense->i_q = i_q;
}
...
void currentSense_Clark_Park_IaIc(float Ia, float Ic, float *Id, float *Iq, float e_angle)
{
   float Ib = -(Ia + Ic);
  ...
  currentSense.i_alpha = I_alpha;
  currentSense.i_beta = I_beta;
...
extern MovingAverageFilter ma_filter_ia;
extern MovingAverageFilter ma_filter_ib;
int g_cnt_t = 0;
void currentSense_ADC_Callback(ADC_HandleTypeDef *hadc) {
  ...
  if (hadc->Instance == ADC1) {
    ...
    if (currentSense.isCalibrating) {
      // currentSense_3currentCalibrationVoltage(&currentSense);
      currentSense_currentCalibrationVoltage(&currentSense);
    } else {
      ...
      if (!motor.currentSense->isReady)
        ...
        // currentSense_3currentVoltage(&currentSense);
        currentSense_currentVoltage(&currentSense);
          currentSense.i_a = -currentSense.i_a;
          currentSense.i_b = -currentSense.i_b;
          //
          // currentSense.i_c = -currentSense.i_c;
        ...
        currentSense.i_b = lpf_process(&motor.lpf_ib0, currentSense.i_b);
          // currentSense.i_c = lpf_process(&motor.lpf_ib0, currentSense.i_c);

          // currentSense.i_a = LowPassFilter(&motor.lpf_ia, currentSense.i_a);
          // currentSense.i_b = LowPassFilter(&motor.lpf_ib, currentSense.i_b);


          //mean filter
          // currentSense.i_a = process_moving_average(&ma_filter_ia, currentSense.i_a);
          // currentSense.i_b = process_moving_average(&ma_filter_ib, currentSense.i_b);

        // Clark_Park_TransformIaIc(currentSense.i_a, currentSense.i_b,
        //                      &currentSense.i_d, &currentSense.i_q,
        //                      motor.e_angle);
        currentSense_Clark_Park_IbIc(currentSense.i_a, currentSense.i_b,
                             &currentSense.i_d, &currentSense.i_q,
                             motor.e_angle);
          // currentSense_Clark_Park_IbIc(currentSense.i_a, currentSense.i_b,
          //                      &currentSense.i_d, &currentSense.i_q,
        ...
        // currentSense.i_q = lpf_process(&motor.lpf_iq0, currentSense.i_q);
          ...
          // currentSense.i_d = LowPassFilter(&motor.lpf_id, currentSense.i_d);
          // currentSense.i_q = LowPassFilter(&motor.lpf_iq, currentSense.i_q);
        motor.CurrentSense.i_a = currentSense.i_a;
        motor.CurrentSense.i_b = currentSense.i_b;
        motor.CurrentSense.i_alpha = currentSense.i_alpha;
        motor.CurrentSense.i_beta = currentSense.i_beta;
        ...
        motor.currentSense->i_q = currentSense.i_q;
          ...
          gMonitorVars.Ia = currentSense.i_a;
          gMonitorVars.Ib = currentSense.i_b;
          gMonitorVars.Ic = currentSense.i_c;
          gMonitorVars.Iq = currentSense.i_q;
          gMonitorVars.Id = currentSense.i_d;
...
float fTest_adc_buffer[100];
int adc_ok = 0;
void test_adc_callback(ADC_HandleTypeDef *hadc) {
  if (hadc->Instance == ADC1) {
    HAL_GPIO_TogglePin(GPIOD, GPIO_PIN_12);
    fTest_adc_buffer[1] =
        (float)(HAL_ADCEx_InjectedGetValue(&hadc1, ADC_INJECTED_RANK_1)) * 3.3 /
        65535;
    adc_ok = 1;
  }
}

void HAL_ADCEx_InjectedConvCpltCallback_with_multirate_controller(
    ADC_HandleTypeDef *hadc) {
  if (hadc->Instance == ADC1) {
    // HAL_GPIO_TogglePin(GPIOD, GPIO_PIN_12);
    currentSense_3currentCalibration(&currentSense);

    execute_multirate_control(&MultiRate_Controller);
    if (MultiRate_Controller.current_enable) {
      currentSense_3current(&currentSense);
      currentSense_getCurrentDQ_NotFilter(&currentSense);
    }
  }
}
void HAL_ADCEx_InjectedConvCpltCallback_1_ok(ADC_HandleTypeDef *hadc) {
  if (hadc->Instance == ADC1) {
    // HAL_GPIO_TogglePin(GPIOD, GPIO_PIN_12);
    currentSense_3currentCalibration(&currentSense);
...
void HAL_ADCEx_InjectedConvCpltCallback_0_ok(ADC_HandleTypeDef *hadc) {
  if (hadc->Instance == ADC1) {
    // HAL_GPIO_TogglePin(GPIOD, GPIO_PIN_12);
    currentSense_3currentCalibration(&currentSense);
    if (currentSense.adc_flag == 0) {
...
void HAL_ADCEx_InjectedConvCpltCallback_0(ADC_HandleTypeDef *hadc) {
  if (hadc->Instance == ADC1) {
    // HAL_GPIO_TogglePin(GPIOD, GPIO_PIN_12);
    currentSense_3currentCalibration(&motor.CurrentSense);
    if (currentSense.adc_flag == 0) {
      currentSense_3current(&motor.CurrentSense);
      currentSense.adc_flag = 1;
    }

    // 2.
    //  current_rawValue[0] = HAL_ADCEx_InjectedGetValue(&hadc1,
    //  ADC_INJECTED_RANK_1); current_rawValue[1] =
    //  HAL_ADCEx_InjectedGetValue(&hadc1, ADC_INJECTED_RANK_2);
    //  current_rawValue[2] = HAL_ADCEx_InjectedGetValue(&hadc1,
    //  ADC_INJECTED_RANK_3); adc_cmp_flag = 1;
    // 3.
  }
}
void HAL_ADCEx_InjectedConvCpltCallback(ADC_HandleTypeDef *hadc) {
  ...
  // //2. test
  // test_adc_callback(hadc);
  // if (hadc->Instance == ADC1)
  // {
  //     // HAL_GPIO_TogglePin(GPIOD, GPIO_PIN_12);
  //     currentSense_3currentCalibration(&currentSense);
  //     currentSense_3current(&currentSense);
  //     currentSense.isReady = true;
  //
  //
  // }
}
