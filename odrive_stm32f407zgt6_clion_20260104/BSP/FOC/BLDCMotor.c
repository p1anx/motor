...
MovingAverageFilter ma_filter_ia;
MovingAverageFilter ma_filter_ib;
void BLDCMotor_LowpassFilterInit(BLDCMotor_t *motor) {
    ...
    // 初始化滤波器
    if (!init_moving_average_filter(&ma_filter_ia, filter_buffer, FILTER_SIZE)) {
        // 初始化失败处理
        return;
    }
    if (!init_moving_average_filter(&ma_filter_ib, filter_buffer, FILTER_SIZE)) {
        // 初始化失败处理
        return;
    }
...
void BLDCMotor_currentVelocityOpenloop(BLDCMotor_t *motor, float target_velocity)
{
    ...
    if (motor->currentSense->isReady)
    {
        ...
        float Ts = (float)1.f/CONFIG_PWM_HZ;
        ...
        // float e_angle = _electricalAngle_calibrated(motor->direction, motor->angle, motor->foc_motor.pole_pairs, motor->foc_motor.zero_electric_angle);
        motor->e_angle = g_e_angle;
        // BLDCMotor_getCurrentDQ(motor);
        // BLDCMotor_setPhaseVoltage(motor, motor->foc_motor.voltage_limit, 0, e_angle);
        // BLDCMotor_SVPWM(motor, 3.0f, 0, e_angle);
        BLDCMotor_setPhaseVoltage(motor, CONFIG_OPENLOOP_VOLTAGE, 0,motor->e_angle);
        // uint32_t t1 = getUs();
...
int BLDCMotor_currentVelocityAngleClosedLoopBandwith(BLDCMotor_t *motor, float target)
{
    ...
    if (motor->currentSense->isReady)
    {
        ...
        gMonitorVars.target = motor->target;
        gMonitorVars.degree = motor->degree;
        gMonitorVars.velocity = motor->velocity;
        gMonitorVars.target_velocity = motor->target_velocity;
        gMonitorVars.target_iq = motor->target_iq;

        // gMonitorVars.Iq = motor->currentSense->i_q;
        // int t1 = getUs();
