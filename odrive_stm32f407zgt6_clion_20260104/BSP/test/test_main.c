...
int main0(void) {
    ...
    while (1) {
        ...
        volatile int counter = 0;  // volatile 确保不被优化
        for (int i = 0; i < 4; i++) {
            counter++;  // 确保循环不会被完全优化掉
        }

        uint32_t cycles = DWT->CYCCNT;
        float time_us = cycles * 1e6f / 168000000.0f;

        printf("Real cycles: %lu, Time: %.2f μs\n", cycles, time_us);
