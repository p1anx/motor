  ...
  if (hadc->Instance == ADC1) {
    ...
    if (currentSense.isCalibrating) {
      ...
      // if (!motor.currentSense->isReady)
        ...
        //                      motor.e_angle);
          currentSense_Clark_Park_IaIb(currentSense.i_a, currentSense.i_b,
                               &currentSense.i_d, &currentSense.i_q,
                               motor.e_angle);
        ...
        motor.CurrentSense.i_q = currentSense.i_q;
          ...
          // buffer[4] = motor.Driver.Ua;
          // buffer[5] = motor.Driver.Ub;
          // buffer[6] = motor.Driver.Uc;
          // VOFA_SendJustFloat(buffer, 7);
          // buffer[0] = motor.e_angle;
          ...
          // buffer[5] = motor.Driver.Ub;
          // buffer[6] = motor.Driver.Uc;
          // VOFA_SendJustFloat(buffer, 3);
           // BLDCMotor_Monitor(&motor, Monitor_Velocity);
          // BLDCMotor_Monitor(&motor,Monitor_TargetIdIq);
          // BLDCMotor_Monitor(&motor,Monitor_IaIbIdIq);
          // BLDCMotor_Monitor(&motor,Monitor_IaIbIdIqUabc);
          // BLDCMotor_Monitor(&motor,Monitor_IalphaIbetaUabc);
          // BLDCMotor_Monitor(&motor,Monitor_UaUbUc);
          BLDCMotor_Monitor(&motor,motor.monitorType);

          // buffer[0] = currentSense.v_a;
          // buffer[1] = currentSense.v_b;
          // buffer[2] = currentSense.v_c;
          // // buffer[3] = motor.currentSense->i_q;
          // VOFA_SendJustFloat(buffer, 4);

          // buffer[0] = motor.Driver.Ua;
          // buffer[1] = motor.Driver.Ub;
          // buffer[2] = motor.Driver.Uc;
          // VOFA_SendJustFloat(buffer, 3);
          ...
          // if (tx_counter++ % 10 == 0) {
          //   UART_DMASendVOFA_justFloat4(motor.target, motor.Driver.Ua, motor.Driver.Ub, motor.Driver.Uc);
          // }
