...
// #define _2PI   2*3.14159265358979323846f

/* 私有函数 */
static inline void MT6835_CS_Low(MT6835_t *mt6835)
{
    HAL_GPIO_WritePin(mt6835->cs_port, mt6835->cs_pin, GPIO_PIN_RESET);
}

static inline void MT6835_CS_High(MT6835_t *mt6835)
{
 ...
 * @param  cs_pin: CS引脚编号
 */
void MT6835_InitStruct(MT6835_t *mt6835, SPI_HandleTypeDef *hspi,
                 GPIO_TypeDef *cs_port, uint16_t cs_pin, int direction, float delta_t)
{
    float last_angle;
    mt6835->hspi = hspi;
    ...
    mt6835->direction = direction;
    mt6835->delta_t = delta_t;
    if (HAL_SPI_Init(hspi) != HAL_OK) {
        printf("mt6835: SPI Init Error\n");
    }
...
void MT6835_Init(MT6835_t *mt6835)
{
    // mt6835->hspi = hspi;
    // mt6835->cs_port = cs_port;
    // mt6835->cs_pin = cs_pin;
    ...
    mt6835->angle_deg = 0.0f;
    mt6835->angle_rad = 0.0f;
    mt6835->hspi->Init.CLKPolarity = SPI_POLARITY_HIGH;
    mt6835->hspi->Init.CLKPhase = SPI_PHASE_2EDGE;
    mt6835->hspi->Init.DataSize = SPI_DATASIZE_8BIT;
    // mt6835->direction = direction;
    // mt6835->delta_t = delta_t;
    if (HAL_SPI_Init(mt6835->hspi) != HAL_OK) {
        printf("mt6835: SPI Init Error\n");
    }
 ...
 * @return 寄存器值(8位)
 */
uint8_t MT6835_ReadRegister(MT6835_t *mt6835, uint16_t reg_addr)
{
    uint8_t tx_data[3];
    uint8_t rx_data[3];
 ...
 */
void MT6835_WriteRegister(MT6835_t *mt6835, uint16_t reg_addr, uint8_t data)
{
    uint8_t tx_data[3];
 ...
 */
bool MT6835_ReadAngle(MT6835_t *mt6835)
{
 ...
 * @return true: 成功, false: 失败
 */
bool MT6835_ReadAngleFast(MT6835_t *mt6835)
{
    uint8_t tx_data[6];
    uint8_t rx_data[6];
...
int MT6835_ReadRawAngle(MT6835_t *mt6835)
{
    uint8_t angle_h, angle_m, angle_l;
    ...
    if (mt6835->direction)
    {
        mt6835->angle_raw = 2097152 - mt6835->angle_raw;
    }

    return mt6835->angle_raw;
}
float MT6835_GetAngleHz(MT6835_t *mt6835)
{
    MT6835_ReadRawAngle(mt6835);
    mt6835->angle_Hz = (float)mt6835->angle_raw / (MT6835_ANGLE_MAX + 1);
    return mt6835->angle_Hz;
}
/**
 * @brief  获取角度值(度)
 * @param  mt6835: MT6835句柄
 * @return 角度值(0-360度)
 */
float MT6835_GetAngleDegrees(MT6835_t *mt6835)
{
 ...
 * @return 角度值(0-2π弧度)
 */
float MT6835_GetAngleRadians(MT6835_t *mt6835)
{
    MT6835_ReadRawAngle(mt6835);
    mt6835->angle_rad = (float)mt6835->angle_raw * 2 * PI / (MT6835_ANGLE_MAX + 1);
...
float g_vel;
float g_t_open;
float g_pi = 2*3.14159265358979323846f;
float g_circle, g_circle_prev;
float MT6835_GetVelocityHz(MT6835_t *mt6835)
{
    ...
    g_angle = delta_angle;
    g_circle_prev = last_circle;
    if (fabs(delta_angle) > 0.6f * _2PI)
    {
        if (delta_angle > 0)
        {
            circle--;
    ...
    g_total_angle = total_angle;
    // g_vel = velocity;
    g_t_open = delta_t;
    g_vel = g_total_angle / g_t_open / _2PI;
...
float MT6835_GetVelocityRPM(MT6835_t *mt6835)
{
    return MT6835_GetVelocityHz(mt6835)*60.0f;
}
float MT6835_GetVelocityDegrees(MT6835_t *mt6835)
{
    return MT6835_GetVelocityHz(mt6835) * 360.0f;
}
/**
 * @brief  获取原始角度值
 * @param  mt6835: MT6835句柄
 ...
 */
uint32_t MT6835_GetAngleRaw(MT6835_t *mt6835)
{
    return mt6835->angle_raw;
}
 ...
 */
void MT6835_SetZeroPosition(MT6835_t *mt6835, uint16_t zero_pos)
{
...
void MT6835_MotorAngle_Init(MT6835_t *mt6835, float gear_ratio)
{
...
void MT6835_Angle_Update_with_last(MT6835_t *mt6835)
{
...
void MT6835_Angle_Update(MT6835_t *mt6835)
{
    // Motor_Angle_t motor = mt6835->motor_angle;
 ...
 * @return 输出轴角度
 */
float MT6835_GetMotorAngleDegrees(MT6835_t *mt6835)
{
...
float g_angle = 0.0f;
void MT6835_Example_BasicUsage(void)
{
...
//===============================================
    ...
    MT6835_InitStruct(&mt6835, mt6835.hspi, mt6835.cs_port, mt6835.cs_pin, mt6835.direction, mt6835.delta_t);
    ...
    while(1)
    {
        ...
        // float mt6835_angle = MT6835_GetAngleHz(&mt6835);
        float mt6835_angle = MT6835_GetAngleRadians(&mt6835);
        g_angle = mt6835_angle;
        // float velocity = MT6835_GetVelocityHz(&mt6835);
        float velocity = MT6835_GetVelocityRPM(&mt6835);
        // printf("velocity: %f\r\n", velocity);

        // 4. 显示结果
        printf("mt6835 rad_vel: %.4f, %f\n",mt6835_angle, velocity);
