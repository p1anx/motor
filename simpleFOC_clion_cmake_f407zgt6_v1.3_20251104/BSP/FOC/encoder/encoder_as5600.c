...
bool AS5600_InitStruct(AS5600_t *sensor)
{
    ...
    // 方法1：不使用方向引脚（软件控制方向）
    if (AS5600_Init(sensor, &as5600_i2c, AS5600_DEFAULT_ADDRESS))
    {
        printf("[OK] AS5600 initialized successfully\n");
    }
    else
    {
        printf("[ERROR] AS5600 initialization failed\n");
        return false;
    }

    // 检查磁铁
    if (AS5600_DetectMagnet(sensor))
    {
        printf("[OK] Magnet detected\n");

        if (AS5600_MagnetTooStrong(sensor))
        {
            printf("Warning: Magnet too strong\n");
        }
        if (AS5600_MagnetTooWeak(sensor))
        {
    ...
    // 配置AS5600
    AS5600_SetPowerMode(sensor, AS5600_POWERMODE_NOMINAL);
    AS5600_SetHysteresis(sensor, AS5600_HYST_LSB1);
    AS5600_SetSlowFilter(sensor, AS5600_SLOW_FILT_4X);
    AS5600_SetFastFilter(sensor, AS5600_FAST_FILT_LSB6);

    // 设置零位
    uint16_t currentAngle = AS5600_RawAngle(sensor);
    // AS5600_SetZPosition(sensor, currentAngle);
...
float AS5600_GetAngularSpeed(AS5600_t *sensor, uint8_t mode, bool update)
{
    ...
    if (update)
    {
        ...
        sensor->angle = sensor->lastReadAngle * AS5600_RAW_TO_RADIANS;
        if (sensor->error != AS5600_OK)
        {
            return 0; // 或者返回 NAN
        }
    ...
    // if (speed_debug++ % 1000 == 0) {
    //     printf("[Speed] now=%lu last=%lu deltaT=%lu angle=%d lastA=%d deltaA=%d\n",
    //            now, sensor->lastMeasurement, deltaT, angle, sensor->lastAngle, deltaA);
    // }
    //
    // // 【关键修复】第一次调用时，初始化时间戳并返回0
    // if (sensor->lastMeasurement == 0 || deltaT == 0 || deltaT > 1000000) {
    //     sensor->lastMeasurement = now;
    //     sensor->lastAngle = angle;
    //     printf("[Speed] First call or invalid deltaT, returning 0\n");
    //     return 0;
    // }

    // // 【修复5】第一次调用或异常时间间隔，初始化并返回0
    // if (sensor->lastMeasurement == 0 || deltaT == 0 || deltaT > 1000000) {
    //     sensor->lastMeasurement = now;
    //     sensor->lastAngle = angle;
    //     return 0;
...
float AS5600_GetAngularSpeed_with_timer(AS5600_t *sensor, uint8_t mode, bool update)
{
    if (!sensor)
        return 0;
...
int Encoder_AS5600_Init(AS5600_t *as5600, I2C_HandleTypeDef *hi2c)
{
    // 初始化I2C（假设已经在CubeMX中配置）
    ...
    printf("Zero position set to: %d(ZPosition NOT SET)\n", currentAngle);
    return 0;
...
void AS5600_Example_Init(void)
{
    ...
    // 方法1：不使用方向引脚（软件控制方向）
    if (AS5600_Init(&magneticEncoder, &hi2c1, AS5600_DEFAULT_ADDRESS))
    {
...
// 读取角度示例
void AS5600_Demo_ReadAngle(AS5600_t *as5600)
{
    // 读取原始角度（0-4095）
    uint16_t rawAngle = AS5600_RawAngle(as5600);
...
void as5600_example_0(void)
{
    AS5600_t as5600;
    AS5600_InitStruct(&as5600);
    while (1)
    {

        AS5600_Demo_ReadAngle(&as5600);
        delay_ms(1000);
